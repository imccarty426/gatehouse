...
export interface RelayUpstream { url: string; toolDenylist?: string[]; alertTools?: string[] }
...
export function loadRelayConfig(path: string): RelayConfig {
  ...
  for (const [name, p] of Object.entries<any>(doc.providers)) {
    ...
    for (const [un, u] of Object.entries<any>(p.upstreams)) {
      ...
      if (u.alertTools !== undefined && (!Array.isArray(u.alertTools) || u.alertTools.some((x: any) => typeof x !== "string")))
        throw new Error(`relay config: provider ${name}: upstream ${un}.alertTools must be an array of strings`);
    }
  }
